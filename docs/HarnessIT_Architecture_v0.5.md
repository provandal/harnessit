...
Every other component of the harness is improvable. Evals are how you know whether a change improved the agent or made it worse. A harness without evals is a demo. A harness with evals is a product. This is non-negotiable.
...
Wrapping the harness are two cross-cutting concerns that keep it honest. Evals run continuously in CI. Trajectories are captured for every run. Telemetry on the harness itself — token usage, latency, tool-call patterns, failure rates — is collected and reviewed. The harness is treated as a product, not a research artifact.
...
**Adopted in two stages, not from the start.** v0.4 committed to self-hosted Langfuse from stage 0; v0.5 retracts that commitment in favor of staged adoption: managed Langfuse Cloud for build stages 0–3 (free tier comfortably covers initial development scale), then self-hosted Langfuse from stage 4 onward when the trajectory viewer ships. The change is informed by the 2026-05-01 four-reviewer pressure test (synthesis §1.6, three reviewers convergent) which flagged self-hosted-Langfuse-at-stage-0 as a substrate-burden contributor that breaks the clone-and-run promise at the moment reader-abandonment risk is highest. Staged adoption preserves Langfuse-as-tracing-substrate, preserves the eventual self-hostability demonstration (now at stage 4 rather than stage 0), and removes the operational tax on stage 0 that competes with everything else stage 0 has to do.

**The trajectory viewer is the natural transition point.** Stage 4 (Build Plan v0.3 §2.1) ships the sequence-diagram trajectory viewer, which queries Langfuse via API. The query path is the same against managed or self-hosted backends — the API contract is stable. Stage 4's deliverable absorbs the self-host transition: a documented `docker compose up`-style setup mirroring the production Langfuse v3.x deployment shape (Postgres + ClickHouse + Redis + MinIO + langfuse-web + langfuse-worker; v3 ingests via its own SDK rather than OTLP, so no OpenTelemetry collector container is involved — earlier v0.5 drafts referenced an OTel collector here, which was a documentation error corrected when the self-host stack was actually built at stage 4b on 2026-05-07), plus a one-time decision about stage-0-through-3 trace continuity (data migration from the Cloud project, or clean cut-over with stage-0-through-3 traces archived in Cloud and stage-4-onward traces flowing to self-hosted). This makes the substrate-substitution lesson concrete: readers see the same investigation logic, the same trajectory viewer, the same trace structure, across two materially different telemetry substrates. Same architectural pattern as the Doppelgänger → AIR Substrate Adapter transition (§6.1, stage 13), one stage earlier.

**What this preserves and what it changes about the architecture's "clone-and-run" claim.** Preserves: tracing-as-consume-not-rebuild; Langfuse-as-the-tracing-substrate-of-choice; the eventual self-hostability demonstration. Changes: stage-0 readers no longer set up self-hosted Langfuse before any harness work begins. They sign up for a Langfuse Cloud project (free tier) and wire the project's API key into their HarnessIT clone. Setup-friction at stage 0 drops materially. The "genuinely cloneable" wording in v0.4 §9.1 is updated to reflect this: stage 0 is genuinely cloneable in minutes; stage 4 onward is genuinely cloneable in 30–60 minutes including the Langfuse self-host setup. The README owns this two-tier honesty.
...
The mapping is roughly two-build-stages-to-one-editorial-stage on average, with some 1:1 and some 3:1 alignment. The Build Plan owns the canonical mapping table (Build Plan v0.3 §2.4). Readers who want the implementation cadence consult the Build Plan; readers who want the narrative cadence consult the Architecture.
...
| 2 | Sensing posts | First tool, many tools, first skill from TheConstruct. The reader sees the agent become coherent. |
...
| 6 | Discipline posts | Evals, trajectories, the things that make the harness a product. |
...
- **The eval dashboard.** Which scenarios pass, which fail, regression status across versions, eval set growth over time. The discipline tier made visible.
...
The next two documents in this set are the editorial plan for the blog series and the design document for Doppelgänger. With those three in place, the writing and the building begin.
