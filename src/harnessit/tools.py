...
GET_HOST_COUNTERS_SCHEMA: dict[str, Any] = {
    ...
    "description": (
        ...
        "the NIC if_index, the count of packets dropped at the host's "
        "PHY layer — i.e., packets that arrived corrupted on the "
        "host's incoming link — and a derived drops_per_million rate "
        "(drops as a fraction of packets the host's upstream leaf "
        "transmitted toward it, in parts per million). Zero counts are "
        "surfaced as 0, never missing — observed-zero is data, not "
        "absence. drops_per_million is null when the host received no "
        "inbound traffic this run (denominator is zero); a numeric "
        "value of 0.0 means the host saw traffic and zero drops. This "
        "is the diagnostic surface for link-layer silent drops (CRC "
        "errors, optical degradation, cable issues): switch-side drops "
        "in get_fabric_counters' dropped_packets track admission "
        "failures, host-side drops here track PHY corruption. The two "
        "together cover both classes. No arguments."
    ),
    "input_schema": {
        "type": "object",
        "properties": {},
